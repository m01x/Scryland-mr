import { Injectable } from '@nestjs/common';
import { TARGET_STORES } from '../../stores/stores.constants';
import { StoreHttpService } from '../../stores/store-http.service';
import { ShopifySuggestAdapter } from './shopify-suggest.adapter';

/**
 * Adapter de Ineko (Shopify). Consulta `/search/suggest.json` y normaliza los
 * productos a ofertas con `store = 'ineko'`.
 */
@Injectable()
export class InekoAdapter extends ShopifySuggestAdapter {
  constructor(http: StoreHttpService) {
    super(http, TARGET_STORES.ineko.baseUrl, TARGET_STORES.ineko.id);
  }
}
